...
List<Map<String, Object>> queryHistoryDataWithTime(List<String> tagList, String startTime, String endTime, DataService dataService) {
    ...
    try {
        ...
        "AND a.starttime ='" + startTime + "' AND a.endtime = '" + endTime + "' limitpage 1,50";
...
List<String> getCopTagList(String copType, String periodType) {
    ...
    String prefix = "";
    if (copType.equals("HP_COP")) {
        prefix = "HP_COP";
    } else if (copType.equals("WSHP_COP")) {
        prefix = "WSHP_COP";
    } else {
        prefix = "COP";
    }
    ...
    switch(periodType) {
        case "oneday":
        case "singleday":
            suffix = "_hour_zizhi"; // 小时数据统一使用此后缀
            break;
        case "sevendays":
        case "week":
        case "day":
            suffix = "_day_zizhi"; // 日数据统一使用此后缀
            break;
        case "month":
            suffix = "_month_zizhi";
            break;
...
Map<String, Object> processMonthDays(LocalDateTime monthTime, String copType, DataService dataService) {
    ...
    // 查询从下一天0点到下个月第一天0点+6分钟的数据
    String queryStartTime = monthStart.plusDays(1).format(sDF);
    String queryEndTime = monthEnd.plusDays(1).plusMinutes(6).format(sDF);
