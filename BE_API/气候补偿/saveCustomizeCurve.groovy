...
// --------------------- 归档当前使用中的曲线 ---------------------
// 切换前，EF 中的曲线属于 useUuidMySQL 对应的记录，先把它同步回 MySQL 备份。
if (useUuidMySQL != null && !useUuidMySQL.equals("")) {
    boolean syncOldResult = syncCurveFromEFToMySQL(useUuidMySQL, hpTotalRunMode);
    if (!syncOldResult) {
        paramMap.put("operationType", "同步");
        paramMap.put("operationContent", "气候补偿页面-切换自定义曲线前同步当前曲线失败-曲线ID:" + useUuidMySQL);
        Map<String, String> solResultMap = (Map<String, String>) sol.execute(param);
    }
}

def isEditableCustomizeCurve = { String curveId, String weatherMode ->
    if (curveId == null || curveId.equals("")) {
        return false;
    }

    String selectSql = "SELECT id FROM sjmg_weather_compensate WHERE `id` = '" + curveId + "' " +
        "AND weather_compensate_mode = '" + weatherMode + "' AND weather_compensate_type = '自定义'";
    List<Map<String,Object>> rows = dynamicDataSource.excuteTenantSqlQuery(selectSql, dbCode);
...
if (curveDataList.size() > 4) {
    data.put("state", "fail");
    data.put("message", "高级调节曲线数量异常");
    return data;
}
...
if (hpTotalRunMode == "1") {
    for (Map<String, Object> curveDataMap : curveDataList) {
        ...
        if (curveId == null || curveId == "") {
            ...
            boolean editableCurve = false;
            try {
                editableCurve = isEditableCustomizeCurve(curveId, "制热");
            } catch (Exception e) {
                editableCurve = false;
            }
    ...
    for (Map<String, Object> curveDataMap : curveDataList) {
        ...
        if (curveId == null || curveId == "") {
            ...
            boolean editableCurve = false;
            try {
                editableCurve = isEditableCustomizeCurve(curveId, "制冷");
            } catch (Exception e) {
                editableCurve = false;
            }
            ...
            // 修改气候补偿曲线
            String updateSql =  "UPDATE sjmg_weather_compensate SET weather_compensate_curve = '" + curveValueJson + "' WHERE `id` = '" + curveId + "' " +
            "AND weather_compensate_mode = '制冷' AND weather_compensate_type = '自定义'";
            String result = dynamicDataSource.excuteTenantSql(updateSql, dbCode);
...
if (useAdvancedAdjustment.equals("1") && (useCustomizeCurveId == null || useCustomizeCurveId.equals(""))) {
    data.put("state", "fail");
    data.put("message", "高级调节选中曲线异常");
    return data;
}
...
if (!useUuid.equals(useUuidMySQL)) {
    ...
    // --------------------- 等待 EF 曲线稳定 ---------------------
    // writeWeatherCompensateCurve 已返回 success，EF 点位刷新需要短暂时间。
    Thread.sleep(500);

    // --------------------- 归档新曲线 ---------------------
    // 确认 EF 稳定后，把 EF 当前权威曲线同步到新 useUuid 的 MySQL 备份记录。
    boolean syncNewResult = syncCurveFromEFToMySQL(useUuid, hpTotalRunMode);
    if (!syncNewResult) {
        paramMap.put("operationType", "同步");
        paramMap.put("operationContent", "气候补偿页面-切换自定义曲线后同步新曲线失败-曲线ID:" + useUuid);
        Map<String, String> solResultMap = (Map<String, String>) sol.execute(param);
    }
}
