/* algorithmProcessId: queryWaterConsumption
 *
 * 用水统计查询：按日/月/年返回用水量趋势；支持环比/同比。
 *
 * 入参：
 * - cycle: 日 | 月 | 年
...
try {
    String detailSql = "SELECT stat_date, SUM(water_value) AS water_value " +
            "FROM sjmg_water_daily_detail " +
    ...
    for (Map<String, Object> row : rows) {
        ...
        Double waterValue = toScaledDouble(row.get("water_value"));
        if (waterValue == null) continue;
        currentTotals.set(idx, currentTotals.get(idx) + waterValue);
    }
} catch (Exception e) {
    data.put("state", "fail");
    data.put("message", "查询用水量明细失败: " + (e.getMessage() != null ? e.getMessage() : "未知错误"));
    return data;
}
...
if ("QOQ".equals(comparison) || "YOY".equals(comparison)) {
    ...
    try {
        String prevSql = "SELECT stat_date, SUM(water_value) AS water_value " +
                "FROM sjmg_water_daily_detail " +
        ...
        for (Map<String, Object> row : prevRows) {
            ...
            Double waterValue = toScaledDouble(row.get("water_value"));
            if (waterValue == null) continue;
            prevMap.put(key, (prevMap.containsKey(key) ? prevMap.get(key) : 0D) + waterValue);
        }
        ...
        data.put("state", "fail");
        data.put("message", "查询对比用水量失败: " + (e.getMessage() != null ? e.getMessage() : "未知错误"));
        return data;
    }
}
